...
// Fetch weather forecast based on coordinates
function fetchForecast(coordinates) {
  ...
  axios.get(apiUrl).then(displayWeatherForecast).catch(error => {
...
function fetchLocationWeather(position) {
  ...
  axios.get(apiUrl).then(displayCurrentWeather).catch(error => {
      alert("Could not fetch your location's weather. Please try again.");
  });
}
